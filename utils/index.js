...
const ABI = [
	...
	{
		inputs: [{ internalType: "uint256", name: "_rewardFactor", type: "uint256" }],
...
module.exports = {
	createSignature: async (postId) => {
      ...
      `,
		...
		const contract = new web3.eth.Contract(ABI, "0xd9B533f2A958A1E8083b04bFc57CB511C0447cC7");
		...
		const signature = web3.eth.accounts.sign(hash, process.env.PRIVATE_KEY)?.signature;

		return { postId, likesCount, signature };
	},
  addressesAndLinks: () => {
    return{
        CONTRACT: "0xd9B533f2A958A1E8083b04bFc57CB511C0447cC7",
        SUBGRAPH:"https://api.thegraph.com/subgraphs/name/mutahhirkhan/social-blocks"
    }
  } 
}
